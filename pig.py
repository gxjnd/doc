...
def checkArgs():
   ...
   # print(opts)                                                                  "fuzz","ipv6","client-src=","color","v6-rapid-commit",
    ...
    def run(self):
       ...
       while not self.kill_received and not dhcpdos:
            ...
            if MODE_IPv6:
                ...
                dhcp_discover = Ether(src=mymac,dst="ff:ff:ff:ff:ff:ff")/IP(src="0.0.0.0",dst="255.255.255.255")/UDP(sport=68,dport=67)/BOOTP(chaddr=mac2str(m),xid=myxid,flags=0)/DHCP(options=myoptions)
              #  dhcp_req = Ether(src=mymac,dst="ff:ff:ff:ff:ff:ff")/IP(src="0.0.0.0",dst="255.255.255.255")/UDP(sport=68,dport=67)/BOOTP(chaddr=mac2str(localm),xid=localxid,flags=0)/DHCP(options=[("message-type","request"),("server_id",sip),("requested_addr",myip),("hostname",myhostname),"param_req_list=[",'pad',"]","end"])
        ...
        if MODE_IPv6:
            ...
            if DHCP in pkt:
                ...
                if pkt[DHCP] and pkt[DHCP].options[0][1] == 2:
                   ...
                   # dhc = pkt[DHCP]
                   # print_r(dhc)
                    #dt = str(dhc, encoding = "utf-8")  
                    #print()
                    #print("UTF-8 解码：", dhc.decode('UTF-8','strict'))
                    #print("GBK 解码：", str_gbk.decode('GBK','strict'))
                    #print(pkt[DHCP].options[0][1])
